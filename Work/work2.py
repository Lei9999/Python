...
for devices_ip in devices_ip_list:
    ssh_client.connect(devices_ip, username='admin', password='admin')
    ssh_client.exec_command('terminal length 0')
    #ssh_client.exec_command('terminal ex 0')
    ...
    config_file_name = (input_conf_save_dir + devices_ip + now_time + ".txt")
